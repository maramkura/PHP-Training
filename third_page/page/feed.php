    ...
    <div class="row">
        ...
        <?php foreach ($posts as $post): ?>
            <div class="list-group-item">
                <a class="list-group-item-heading" href="index.php?page=feed&username=<?=$post['username']?>"><?=$post['username']?></a>
                <p class="list-group-item-text">
                    (<?=$post['created_at']?>) - <?=$post['body']?>
                    <strong><?=$post['positive_ranking']?></strong>
                    <strong><?=$post['negative_ranking']?></strong>
                    <a href="index.php?page=rank&post_id=<?=$post['id']?>&rank=1" class="btn btn-default btn-sm"><i class="glyphicon glyphicon-thumbs-up"></i></a>
                    <a href="index.php?page=rank&post_id=<?=$post['id']?>&rank=-1" class="btn btn-default btn-sm"><i class="glyphicon glyphicon-thumbs-down"></i></a>
                    <a href="index.php?page=feed&reply_to_post=<?=$post['id']?>" class="btn btn-default btn-sm">REPLY</a>
                </p>
                <?php
